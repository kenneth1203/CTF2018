...
def check32(poly):
    if poly & 0x80000000 == 0:
        ...
        out('         instead, try')
        out('         0x{0:08x} (reversed/lsbit-first)'.format(poly
                                                               | 0x80000000))
        out('         0x{0:08x} (normal/msbit-first)'.format(
            reverseBits(poly | 0x80000000)))
...
def get_parser():
    ...
    subparser = subparsers.add_parser(
        'flip',
        parents=[outfile_parser],
        help=
        "flip the bits to convert normal(msbit-first) polynomials to reversed (lsbit-first) and vice versa"
    )
    ...
    subparser = subparsers.add_parser(
        ...
        parents=[outfile_parser],
        help=
        "find the reciprocal (Koopman notation) of a reversed (lsbit-first) polynomial and vice versa"
    )
    subparser.add_argument('poly', type=str, help='[int] polynomial')
    ...
    subparser = subparsers.add_parser(
        'table',
        parents=[outfile_parser, poly_flip_parser, default_poly_parser],
    ...
    subparser = subparsers.add_parser(
        'reverse',
        parents=[
            outfile_parser, poly_flip_parser, desired_poly_parser,
            default_accum_parser, default_poly_parser
        ],
        help=
    ...
    subparser = subparsers.add_parser(
        'undo',
        parents=[
            outfile_parser, poly_flip_parser, accum_parser,
            default_poly_parser, infile_parser
        ],
    ...
    subparser.add_argument(
        ...
        type=str,
        default='0',
        help='[int] number of bytes to rewind [default: 0]')
    subparser.set_defaults(func=undo_callback)

    subparser = subparsers.add_parser(
        'calc',
        parents=[
            outfile_parser, poly_flip_parser, default_accum_parser,
            default_poly_parser, infile_parser
        ],
...
def reverse_callback():
    ...
    for patch in patches:
        out('4 bytes: {{0x{0:02x}, 0x{1:02x}, 0x{2:02x}, 0x{3:02x}}}'.format(
            *patch))
            ...
            for last_4_bytes in patches:
                if all(p in permitted_characters for p in last_4_bytes):
                    ...
                    out('alternative: {1}{2}{3}{4}{5}{6} ({0})'.format(
                        'OK' if calc(patch, accum) == desired else 'ERROR',
                        *map(chr, patch)))
...
def undo_callback():
    ...
    if not 0 < maxlen <= len(data):
        maxlen = len(data)
    out('rewinded {0}/{1} ({2:.2f}%)'.format(
        maxlen, len(data), maxlen * 100.0 / len(data) if len(data) else 100))
